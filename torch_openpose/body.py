...
class Body(object):
    ...
    def compute_paf(self, input_tensor):
        ...
        paf = np.transpose(np.squeeze(paf_tensor_np),
                           (1, 2, 0))  # output 1 is heatmaps
        return paf, paf_tensor

    def compute_heatmap_paf_avg(self, data, oriImg_shape):
        heatmap_avg = np.zeros((oriImg_shape[0], oriImg_shape[1], 19))
        paf_avg = np.zeros((oriImg_shape[0], oriImg_shape[1], 38))
        ...
        heatmap = cv2.resize(
            heatmap, (0, 0), fx=self.stride, fy=self.stride, interpolation=cv2.INTER_CUBIC)
        heatmap = heatmap[:self.imageToTest_padded_shape[0] -
                          self.pad[2], :self.imageToTest_padded_shape[1] - self.pad[3], :]
        heatmap = cv2.resize(
            heatmap, (oriImg_shape[1], oriImg_shape[0]), interpolation=cv2.INTER_CUBIC)
        ...
        paf = paf[:self.imageToTest_padded_shape[0] - self.pad[2],
                  :self.imageToTest_padded_shape[1] - self.pad[3], :]
        paf = cv2.resize(
            paf, (oriImg_shape[1], oriImg_shape[0]), interpolation=cv2.INTER_CUBIC)

        heatmap_avg += heatmap_avg + heatmap
        paf_avg += + paf
        return heatmap_avg, paf_avg

    def image_to_input_tensor(self, imageToTest, set_local_state=False):
        imageToTest_padded, pad = util.padRightDownCorner(
            imageToTest, self.stride, self.padValue)
        im = np.transpose(np.float32(
            imageToTest_padded[:, :, :, np.newaxis]), (3, 2, 0, 1)) / 256 - 0.5
        im = np.ascontiguousarray(im)

        if set_local_state:
            self.imageToTest_padded_shape = imageToTest_padded.shape
            self.pad = pad

        return torch.from_numpy(im).float()

    # TODO: replace with cpp version
    def get_pose(self, heatmap_avg, paf_avg, oriImg_shape):
        all_peaks = []
        peak_counter = 0
        thre1 = 0.1
        thre2 = 0.05
    ...
    def __call__(self, oriImg):
        data = self.image_to_input_tensor(oriImg, True)

        if torch.cuda.is_available() and self.useGPU:
            data = data.cuda()

        heatmap_avg, paf_avg = self.compute_heatmap_paf_avg(data, oriImg.shape)
        return self.get_pose(heatmap_avg, paf_avg, oriImg.shape)
